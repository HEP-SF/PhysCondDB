...
public interface GlobalTagMapBaseRepository extends CondDBPageAndSortingRepository<GlobalTagMap, Long>, JpaSpecificationExecutor<GlobalTagMap> {
	...
	@Query("SELECT distinct p FROM GlobalTagMap p JOIN FETCH p.globalTag g JOIN FETCH p.systemTag t WHERE p.globalTag.name = (:globaltag) and p.systemTag.name like (:tag)")
	List<GlobalTagMap> findByGlobalTagAndTagNameLike(@Param("globaltag")String gtag, @Param("tag")String tag);
