...
public class GlobalTagExpRestController extends BaseController {
	...
	@ApiOperation(value = "Create a GlobalTag.", notes = "Input data are in json, and should match all needed fields for a new global tag.", response = GlobalTag.class)
	public Response createGlobalTag(@Context UriInfo info, GlobalTag globaltag) throws ConddbWebException {
		try {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Update a GlobalTag.", notes = "Input data are in json, and should match all  fields to be updated.", response = GlobalTag.class)
	public Response updateGlobalTag(@Context UriInfo info,
			@ApiParam(value = "id: id of the globaltag to be updated", required = true) @PathParam("id") String id,
			Map<?, ?> map) throws ConddbWebException {
		...
		try {
			...
			if (existing.islocked()) {
				if (map.containsKey("lockstatus")
						&& String.valueOf(map.get("lockstatus")).equalsIgnoreCase(GlobalTagStatus.UNLOCKED.name())) {
					// Unlock the global tag and return immediately....no other
					// action is allowed.
					existing.lock(false);
					map.clear();
				} else {
			...
			// TODO: cannot update the name of a global tag...may be create a
			// REST method on purpose ???
			// if (map.containsKey("name")) {
			// List<GlobalTagMap> maplist =
			// globalTagService.getGlobalTagMapByGlobalTagName(id);
			// if (maplist != null && maplist.size()>0 || existing.islocked()) {
			// String msg = "Error updating locked globaltag resource: <name>
			// cannot be modified!";
			// throw buildException(msg, msg, Response.Status.NOT_MODIFIED);
			// }
			...
			if (map.containsKey("snapshotTime")) {
				Timestamp snapshottime = timestampDeserializer
						.timestampFromString(String.valueOf(map.get("snapshotTime")));
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Map a tag to a globaltag.", notes = "Can be used for one or many tags.", response = GlobalTag.class)
	public Response mapTagsToGlobalTag(@Context UriInfo info,
			@ApiParam(value = "id: id of the globaltag to be updated", required = true) @PathParam("id") String id,
			@ApiParam(value = "action: {addtags(default)|merge}. If merge is used, the input map should contain a global tag name. "
					+ "All associated tag will then be also associated to the global tag identified by {id}.", required = false) @DefaultValue("addtags") @QueryParam("action") final String action,
			Map<?, ?> map) throws ConddbWebException {
	...
	@DELETE
	@ApiOperation(value = "Delete a globaltag.", notes = "It should be used one global tag at the time. This method is meant for administration purposes.", response = GlobalTag.class)
	public Response deleteGlobalTag(@Context UriInfo info,
			@ApiParam(value = "id: id of the globaltag to be deleted", required = true) @PathParam("id") String id)
